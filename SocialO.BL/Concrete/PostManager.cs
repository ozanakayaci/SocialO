...
namespace SocialO.BL.Concrete
{
	public class PostManager : ManagerBase<Post>, IPostManager
	{
		...
		public async Task<IEnumerable<GetPostDto>> GetAllPostById(int followerId, int page, int pageSize, bool isOwnPost)
		{
			var posts =  await base.repository.dbContext.Posts
				.Where(p => p.User.Following
					.Any(f =>   isOwnPost ? (f.UserId == followerId) : (f.FollowerId == followerId || f.UserId == followerId)))
